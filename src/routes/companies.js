...
router.post('/:id/categories', authenticate, (req, res, next) => {
  ...
  const { name, type = 'despesa', color = '#f5f4f0', account_code: accountCode } = req.body;
  ...
  const col = String(color || '').trim().slice(0, 7) || '#f5f4f0';
...
router.patch('/:id/categories/:catId',
  ...
  async (req, res) => {
    ...
    if (updates.color != null) {
      updates.color = String(updates.color).trim().slice(0, 7) || '#f5f4f0';
    }
